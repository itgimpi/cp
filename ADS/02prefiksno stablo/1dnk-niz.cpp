...
У случају када је азбука мала (што је случај у овом задатку), 
пресликавање карактера у наредне чворове дрвета можемо чувати у низу 
(на местима карактера који немају наследнике чуваћемо null показиваче). 
Пошто је за брисање битно да знамо колико наследника има неки чвор, 
у чвору ћемо чувати и ту информацију. 
Функције за убацивање и претрагу дрвета једноставно можемо имплементирати и итеративно. 
Са друге стране, избацивање није једноставно имплементирати итеративно 
(јер је након брисања речи потребно вратити се уз дрво и обрисати све празне чворове, 
а немамо показиваче ка родитељским чворовима 
којима бисмо се једноставно могли враћати унатраг).
*/
...
struct Cvor { // cvor prefiksnog stabla...
    Cvor* grane[4]; // 4 grane za 4 moguća slova na toj poziciji / tom nivou
...
Cvor* izbaci(Cvor* cvor, const string& str) { // funkcija izbacuje rec str iz drveta sa korenom koren
    return izbaci(cvor, str, 0); // funkcija vraca koren drveta nastalog nakon izbacivanja
}
